...
Accuracy analysis:
<<>>=
m0.acc.aov<-lmer(acc~dist+RCType+int+(1+dist+RCType+int|subj)+(1+dist+RCType+int|item),questions.RC1,family=binomial())
...
<<>>=
m0.acc<-lmer(acc~dist.SR+dist.OR+RCType+(1+dist.SR+dist.OR+RCType|subj)+(1|item),questions.RC1,family=binomial())  

fixefs<-fixef(m0.acc)
SEs<-sqrt(diag(vcov(m0.acc)))
zs<-fixefs/SEs

e1acc<-round(cbind(fixefs,SEs,zs),digits=2)[2:4,1:3]

rownames(e1acc) <- c("Distance [SR]","Distance [OR]","RC Type")

colnames(e1acc)<-c("coef.","SE","z-score")
@

<<echo=F,results=tex>>=
xtable(e1acc,caption="The effect of distance within subject and object relatives, and the effect of relative clause on question-response accuracy in Experiment 1.",label="e1acc1")
@

Question response RTs analysis:

<<>>=
m0.resp.RT<-lmer(log(rt)~dist.SR+dist.OR+RCType+(1+dist.SR+dist.OR+RCType|subj)+(1+dist.SR+dist.OR+RCType|item),questions.RC1)
@
...
m0<-lmer(lrt~dist+RCType+int+(1+dist+RCType+int|subj)+(1|item),
...
m0.pre<-lmer(lrt~dist+RCType+int+(1+dist+RCType+int|subj)+
...
#xtable(expt1RCVerbresults)
}
@

<<echo=F,results=tex>>=
xtable(precrit.crite1,
...
<<>>=
m2.nested<-lmer(lrt~dist.SR+dist.OR+RCType+(1+dist.SR+dist.OR+RCType|subj)+(1+dist.SR+dist.OR+RCType|item),subset(RC1,region=="RCVerb" & rrt>-10))
...
rownames(expt1RCVerbresultsnested)<-
c("Distance [SR]","Distance [OR]","RC Type")  
...
<<echo=F,results=T>>=
xtable(expt1RCVerbresultsnested,
...
set_cppo('fast')
fitnested <- stan(model_code = expt1_codenested, data = e1data, 
            iter = 500, chains = 4)
...
##log rt:
RC1.uniq.reg<-ddply(RC1, 
                    .(subj, expt, item, cond, region), 
                    summarize, 
                    rt = sum(lrt))

RC1.merged<-merge(RC1.uniq.reg,region.df, 
                  by.x=c("cond","region"))
...
RCType<-factor(ifelse(RC1.merged$cond%in%c("a","b"),"src","orc"),
               levels=c("src","orc"))
Dist<-factor(ifelse(RC1.merged$cond%in%c("a","c"),"distant","near"),
...
## plot log rts:
data.rs <- melt(RC1.merged, 
          id=c("RCType","Dist","cond","region",
               "region.id","subj"), 
                measure=c("rt"),
                na.rm=TRUE)
...
M.id.w <- cast(temp, RCType+Dist+cond+region+region.id  ~ ., 
...
byregion.plot.sr<-function(data,
                        mytitle,k=1,
                        x.lab="position",
...
byregion.plot.or<-function(data,
                        mytitle,k=1,
                        x.lab="position",
                        y.lab="reading time [log ms]"){
  ggplot(data,aes(x=region.id,y=M,
...
## SRC plot:
(plot.src<-byregion.plot.sr(M.id.w.src,
              mytitle="Effect of distance [subject relatives]",k=1,
...
###################################################
### code chunk number 17: HindiExpectationLocalityV8.Rnw:716-720
###################################################

bitmap("e1orbyregion.tiff", height = 4, width = 7, units = 'in', type="tifflzw", res=600)
                                        
(plot.orc<-byregion.plot.or(M.id.w.orc,
...
byregion.plot.rc.distant<-function(data,
                        mytitle,k=1,
                        x.lab="position",
                        y.lab="reading time [log ms]"){
  ggplot(data,aes(x=region.id,y=M,
...
byregion.plot.rc.near<-function(data,
                        mytitle,k=1,
                        x.lab="position",
...
(plot.rctype.dist<-byregion.plot.rc.distant(M.id.w.dist,
                         mytitle="Effect of RC type (distant conditions)",k=1,
                         x.lab="position",y.lab="reading time [msec]")
)


(plot.rctype.near<-byregion.plot.rc.near(M.id.w.near,
                                    mytitle="Effect of RC type (near conditions)",k=1,
                                    x.lab="position",y.lab="reading time [msec]")
)
...
m0.rt.aov<-lmer(log(rt)~dist+exp+int+(1+dist+exp+int|subj)+(1|item),questions.CP1)
...
data.rs <- melt(CP1.uniq.reg.merged, 
                id=c("Exp","Dist","cond","region",
                     "region.id","subj"), 
                measure=c("lrt"),
                na.rm=TRUE)
...
###################################################
### code chunk number 28: HindiExpectationLocalityV8.Rnw:1240-1259
###################################################
...
m02.anova.crit.asfactor<-lmer(lrt~dist+exp+int+(1+dist+exp+int|subj)+(1|item),subset(CP1.critdata,region2=="crit"),
...
## highly correlated:
xyplot(probs~exp|item,CP1.critdata)

m01.anova.crit.asprob<-lmer(lrt~dist*scale(probs,scale=F)+(1|subj)+(0+scale(probs,scale=F)|subj)+(0+dist:scale(probs,scale=F)|subj)+(1|item),subset(CP1.critdata,region2=="crit"))
...
m0.anova.postcrit<-lmer(lrt~dist+exp+int+(1|subj)+(1|item),subset(CP1.critdata,region2=="postcrit"))

m0.anova.postcrit<-lmer(lrt~dist+exp+int+(dist+exp+int-1|subj)+(1|subj)+
                                            (1|item),subset(CP1.critdata,region2=="postcrit"))  


m0.anova.postcrit<-lmer(lrt~dist*scale(probs,scale=F)+(1+dist*scale(probs,scale=F)|subj)+
                          (1+dist*scale(probs,scale=F)|item),
                        subset(CP1.critdata,region2=="postcrit"))  



m0.nested.crit<-lmer(lrt~(dist.exp+dist.noexp+exp)*scale(probs,scale=F)+(1+(dist.exp+dist.noexp+exp)*scale(probs,scale=F)|subj)+(1+dist.exp+dist.noexp+exp|item),subset(CP1.critdata,region2=="crit"))  

m0.nested.postcrit<-lmer(lrt~(dist.exp+dist.noexp+exp)*scale(probs,scale=F)+(1+(dist.exp+dist.noexp+exp)*scale(probs,scale=F)|subj)+(1+dist.exp+dist.noexp+exp|item),subset(CP1.critdata,region2=="postcrit"))  
...
## function for generic by-region plots:
byregion.plot.cp<-function(data,
                        mytitle,k=1,
                        x.lab="position",
                        y.lab="reading time [msec]"){
  ggplot(data,aes(x=region.id,y=M,
...
byregion.plot.sp<-function(data,
                        mytitle,k=1,
                        x.lab="position",
                        y.lab="reading time [msec]"){
  ggplot(data,aes(x=region.id,y=M,
...
###################################################
### code chunk number 42: HindiExpectationLocalityV8.Rnw:1586-1590
###################################################
bitmap("e2cpbyregion.tiff", height = 4, width = 7, units = 'in', type="tifflzw", res=600)
                                        
(plot.exp<-byregion.plot.cp(M.id.w.exp,
                         mytitle="Effect of distance [Complex predicate condition]",k=1,
                         x.lab="position",y.lab="reading time [log ms]")
 )

bitmap("e2spbyregion.tiff", height = 4, width = 7, units = 'in', type="tifflzw", res=600)
(plot.noexp<-byregion.plot.sp(M.id.w.noexp,
...
## function for generic by-region plots:
byregion.plot2.short<-function(data,
                         mytitle,k=1,
                         x.lab="position",
                         y.lab="reading time [msec]"){
  ggplot(data,aes(x=region.id,y=M,
...
byregion.plot2.long<-function(data,
                         mytitle,k=1,
                         x.lab="position",
                         y.lab="reading time [msec]"){
  ggplot(data,aes(x=region.id,y=M,
...
(plot.short<-byregion.plot2.short(M.id.w.short,
                            mytitle="Effect of expectation [near condition]",k=1,
                            x.lab="position",y.lab="reading time [msec]")
 )


###################################################
### code chunk number 46: HindiExpectationLocalityV8.Rnw:1677-1681
###################################################
(plot.long<-byregion.plot2.long(M.id.w.long,
                           mytitle="Effect of expectation [long condition]",k=1,
                           x.lab="position",y.lab="reading time [msec]")
 )


###################################################
### code chunk number 47: HindiExpectationLocalityV8.Rnw:1699-1709
###################################################
...
###################################################
### code chunk number 48: HindiExpectationLocalityV8.Rnw:1712-1713
###################################################
m0<-lmer(lrt~exp+dist+int+(1+exp+dist+int|subj)+(1+exp+dist+int|item),CP1.pos2)
@

\subsection{Stan analysis Expt 2}
...
CP1$dist.cp<-ifelse(CP1$cond%in%c("a"),-1,
                    ifelse(CP1$cond%in%c("b"),1,0))
CP1$dist.sp<-ifelse(CP1$cond%in%c("c"),-1,
                    ifelse(CP1$cond%in%c("d"),1,0))
...
fitnested <- stan(model_code = expt2_codenested, data = e2data, 
            iter = 1000, chains = 2)
